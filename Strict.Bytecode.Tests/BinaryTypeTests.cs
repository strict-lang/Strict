...
public sealed class BinaryTypeTests : TestBytecode
{
	...
	public void WriteAndReadPreservesMethodInstructions()
	{
		...
		var source = new BinaryType(binary, nameof(BinaryTypeTests),
			[new BinaryMember("value", Type.Number, null)], new Dictionary<string, List<BinaryMethod>>
			{
				["Compute"] =
				[
					new BinaryMethod("", [new BinaryMember("input", Type.Number, null)], Type.Number, [
	...
	public void InvalidMagicThrows()
	{
		...
		using var reader = new BinaryReader(stream);
		Assert.That(
			() => new BinaryType(reader, new BinaryExecutable(TestPackage.Instance), Type.Number),
	...
	public void ReconstructMethodNameIncludesParametersAndReturnType() =>
		Assert.That(
			BinaryType.ReconstructMethodName("Compute",
				new BinaryMethod("", [new BinaryMember("first", Type.Number, null)], Type.Number,
					[new ReturnInstruction(Register.R0)])), Is.EqualTo("Compute(first Number) Number"));
}