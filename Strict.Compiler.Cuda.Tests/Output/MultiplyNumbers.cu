...
extern "C" __global__ void Multiply(const float *first, const float *second, float *output, const int count)
{
...
float* executeOperation(const char* operationName);
...
int main(){
    ...
    // Executing kernel 
    Multiply<<<N,1>>>(d_first, d_second, d_output, count);
    ...
    // Verification
    for(int i = 0; i < N; i++){
				assert(fabs(output[i] - 2.0f) < 0.000001f);
    }
