...
public readonly struct ValueInstance : IEquatable<ValueInstance>
{
	...
	{
		IsText => otherType.Name == Base.Text || IsListOfCharacterType(otherType),
	...
	private static bool IsListOfCharacterType(Type type) =>
		type is GenericTypeImplementation { Generic.Name: Base.List } listImpl &&
		listImpl.ImplementationTypes.Count == 1 &&
		listImpl.ImplementationTypes[0].Name == Base.Character;
...
return type is GenericTypeImplementation genTypeImpl &&
genTypeImpl.ImplementationTypes[0].IsSameOrCanBeUsedAs(methodReturnType)
? new ValueInstance(value, number, methodReturnType)
: this;
}
return IsSameOrCanBeUsedAs(
methodReturnType is GenericTypeImplementation genMethodImpl
? genMethodImpl.ImplementationTypes[0]
: methodReturnType)
? new ValueInstance(value, number, methodReturnType)
: this;
	}
	...
	*/
	...
	public double AsNumber() => number;
	public bool AsBool() => number != 0;
	public bool IsTextInstance => number == IsText;
	public bool IsListInstance => number == IsList;
	public bool IsDictionaryInstance => number == IsDictionary;
	public bool IsTypeInstance => number == IsType;
	...
	public object? Value
	{
		get
		{
			if (number == IsText)
				return (string)value;
			if (number == IsList)
				return ((ValueListInstance)value).Items;
			if (number == IsDictionary)
				return ((ValueDictionaryInstance)value).Items;
			if (number == IsType)
				return ((ValueTypeInstance)value).Members;
			var type = (Type)value;
	...
	public ValueInstance? FindInnerValue(string name) =>
		number == IsType && ((ValueTypeInstance)value).Members.TryGetValue(name, out var v)
			? v
			: null;
	...
	public int GetIteratorLength()
	{
		...
		if (number == IsType || number == IsDictionary)
	...
	public static ValueInstance Create(Type returnType, object? val)
	{
		...
		if (IsTextType(effective))
			return new ValueInstance(val is string s2 ? s2 : val?.ToString() ?? "");
	...
	public static ValueInstance Create(Type returnType, ValueInstance val)
	{
		...
		if (IsTextType(effective))
			return new ValueInstance(val.Value is string sv ? sv : val.ToExpressionCodeString().Trim('"'));
	...
	public static ValueInstance CreateObject(Type returnType, object? val)
	{
		...
		if (IsNumberType(effective))
			return new ValueInstance(returnType, EqualsExtensions.NumberToDouble(val));
		if (IsTextType(effective))
			return val is string s ? new ValueInstance(s) : throw new InvalidTypeValue(returnType, val!);
		...
		if (val is Dictionary<string, ValueInstance> typedMembers)
			return new ValueInstance(new ValueTypeInstance(returnType, typedMembers));
		...
		if (val is IDictionary<string, object?> members)
			return new ValueInstance(new ValueTypeInstance(returnType,
				members.ToDictionary(kv => kv.Key,
					kv => kv.Value is ValueInstance mvi ? mvi : kv.Value is string str
	...
	public static bool IsNumberType(Type type) =>
		type.Name == Base.Number || type.Members.Count == 1 &&
	...
	*/
	...
	public static Type GetEffectiveType(Type returnType) =>
		returnType.IsMutable && returnType is GenericTypeImplementation genImpl
			? genImpl.ImplementationTypes[0]
			: returnType;
