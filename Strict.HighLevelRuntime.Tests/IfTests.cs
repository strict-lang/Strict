...
public sealed class IfTests
{
	...
	public void EvaluateIfTrueThenReturn()
	{
		...
		Assert.That(result.Number, Is.EqualTo(33));
	}
	...
	public void EvaluateIsInEnumerableRange()
	{
		...
		var rangeInstance = new ValueInstance(interpreter.rangeType, [
			default,
			new ValueInstance(interpreter.numberType, 1),
			new ValueInstance(interpreter.numberType, 10)
		]);
		var result = interpreter.Execute(t.Methods.Single(m => m.Name == "IsInRange"),
			new ValueInstance(t, 7.0), [rangeInstance]);
		Assert.That(result.Boolean, Is.EqualTo(true));
		result = interpreter.Execute(t.Methods.Single(m => m.Name == "IsInRange"),
			new ValueInstance(t, 11.0), [rangeInstance]);
		Assert.That(result.Boolean, Is.EqualTo(false));
	}
	...
	public void EvaluateIsNotInEnumerableRange()
	{
		...
		var rangeInstance = new ValueInstance(interpreter.rangeType, [
			...
			new ValueInstance(interpreter.numberType, 10.0)
		]);
		var result = interpreter.Execute(t.Methods.Single(m => m.Name == "IsNotInRange"),
			new ValueInstance(t, 11), [rangeInstance]);
		Assert.That(result.Boolean, Is.EqualTo(true));
		result = interpreter.Execute(t.Methods.Single(m => m.Name == "IsNotInRange"),
