...
namespace Strict.Language.Tests
{
    public class PackageTests
    {
        ...
        public void NoneAndBooleanAreAlwaysKnown()
        {
            ...
            Assert.That(
                emptyPackage.FindType(nameof(NoneAndBooleanAreAlwaysKnown), emptyPackage),
                Is.Null);
        }
        ...
        public void GetFullNames()
        {
            ...
            Assert.That(mainType.ToString(),
                Is.EqualTo(nameof(TestPackage) + "." + mainType.Name));
        ...
        {
            Assert.That(mainType.GetType(publicSubType.ToString()),
                Is.EqualTo(publicSubType));
        ...
        public void LoadingTypesOverAndOverWillAlwaysQuicklyReturnTheSame()
        {
            ...
            for (var index = 0; index < 1000; index++)
                if (otherMainPackage.FindType(mainType.Name, otherMainPackage).Name !=
                    mainType.Name)
                    throw new AssertionException("FindType=" + //ncrunch: no coverage
                        otherMainPackage.FindType(mainType.Name, otherMainPackage) +
                        " didn't find " + mainType);
        }
    }
}