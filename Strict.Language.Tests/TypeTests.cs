...
namespace Strict.Language.Tests
{
    public class TypeTests
    {
        ...
        public void AddingTheSameNameIsNotAllowed() =>
            Assert.Throws<Type.TypeAlreadyExistsInPackage>(() =>
                new Type(package, "App", null));

        [Test]
        public void EmptyLineIsNotAllowed() =>
            Assert.That(
                Assert.Throws<Type.ParsingFailed>(() =>
                    new Type(package, Base.Count, null).Parse("\n")).InnerException,
                Is.TypeOf<Type.EmptyLine>());
        ...
        public void WhitespacesAreNotAllowed()
        {
            Assert.That(
                Assert.Throws<Type.ParsingFailed>(() =>
                    new Type(package, Base.Count, null).Parse(" ")).InnerException,
                Is.TypeOf<Type.ExtraWhitespacesFoundAtBeginningOfLine>());
            Assert.That(
                Assert.Throws<Type.ParsingFailed>(() =>
	...
	log.WriteLine")).InnerException,
        ...
        public void JustMembersIsNotValidCode() =>
            Assert.That(
                Assert.Throws<Type.ParsingFailed>(() =>
                    new Type(package, Base.Count, null).Parse(
                        new[] { "has log", "has count" })).InnerException,
                Is.TypeOf<Type.NoMethodsFound>());

        [Test]
        public void InvalidSyntax() =>
            Assert.That(
                Assert.Throws<Type.ParsingFailed>(() =>
                        new Type(package, Base.Count, null).Parse(new[]
                            {
                                "has log", "a b"
                            })).
