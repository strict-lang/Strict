...
namespace Strict.Language
{
    ...
    public class Type : Context
    {
        ...
        /// </summary>
        public Type(Package package, string name, ExpressionParser expressionParser) : base(
            package, name)
        {
        ...
        {
            public ExtraWhitespacesFoundAtBeginningOfLine(string text, int line,
                string method) : base(text, line, method) { }
        }

        public class ExtraWhitespacesFoundAtEndOfLine : ParsingFailedInLine
        {
            public ExtraWhitespacesFoundAtEndOfLine(string text, int line, string method) :
                base(text, line, method) { }
        }

        public abstract class ParsingFailedInLine : Exception
        {
            protected ParsingFailedInLine(string message, int line, string method) : base(
                message)
            {
                Number = line;
                Method = method;
        ...
        public class NoMethodsFound : ParsingFailedInLine
        {
            public NoMethodsFound(int line, string method) : base(
                "Each type must have at least one method, otherwise it is useless", line,
                method) { }
        }

        private string[] GetAllMethodLines(string definitionLine)
        {
            if (IsTrait && lineNumber + 1 < lines.Length &&
        ...
        public bool IsTrait =>
            Implements.Count == 0 && Members.Count == 0 && Name != Base.Number;
