...
public sealed class TypeParser(Type type, string[] lines)
{
		...
		if (nameAndExpression.MoveNext())
		{
			...
			if (nameAndExpression.Current[0] == EqualCharacter)
			{
				...
				var valueOnly = withIndex >= 0
					...
					: constantValue;
				if (usedKeyword == Keyword.Constant && int.TryParse(valueOnly, out var forcedEnumNumber))
					type.AutogeneratedEnumValue = forcedEnumNumber + 1;
				var member = new Member(type, nameAndType, valueOnly.IsEmpty
					? null
					: GetInitialValueType(parser, nameAndType, valueOnly), LineNumber, usedKeyword)
				{
					InitialValueText = valueOnly.IsEmpty
						? null
						: valueOnly.ToString()
				};
		...
		ReadOnlySpan<char> constantValue)
	{
		...
		// Enum constants are PascalCase and must stay Number/Text/etc. Only camelCase fields
		// named after a type (has character = 48) are upcast to that type.
	...
	{
		// Only wrap camelCase fields (has character = 48 → Character(48)). Enum constants such as
		// `constant Add` after `constant StoreSeparator = 10` stay numeric even if a type named
		// Add exists (Add.from often takes two Numbers, so Add(11) would fail to parse).
		...
		return remainingTextSpan.StartsWith(type.Name) && !char.IsUpper(memberName[0])
