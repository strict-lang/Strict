		...
		string usedKeyword)
	{
		...
		if (nameAndExpression.MoveNext())
		{
			...
			if (nameAndExpression.Current[0] == EqualCharacter)
			{
				...
				return new Member(type, nameAndType, GetMemberExpression(parser, nameAndType,
					constantValue), usedKeyword);
			}
		...
		return IsMemberTypeAny(nameAndType, nameAndExpression)
			? throw new MemberWithTypeAnyIsNotAllowed(type, LineNumber, nameAndType)
			: new Member(type, nameAndType, usedKeyword == Keyword.Constant
				? GetMemberExpression(parser, nameAndType, (type.AutogeneratedEnumValue++).ToString())
				: null, usedKeyword);
	}
		...
		string usedKeyword, string nameAndType)
	{
		var member = new Member(type, nameAndType,
			ExtractConstraintsSpanAndValueExpression(parser, remainingLine, nameAndType,
				out var constraintsSpan), usedKeyword);
