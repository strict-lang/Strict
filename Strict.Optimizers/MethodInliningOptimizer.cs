...
{
  public override void Optimize(global::Strict.Bytecode.BinaryExecutable binary)
  {
   foreach (KeyValuePair<string, BinaryType> typeEntry in binary.MethodsPerType)
 ...
 private List<Instruction> InlineInstructions(global::Strict.Bytecode.BinaryExecutable binary,
    string currentTypeName,
    string currentMethodName, List<Instruction> instructions)
  {
 ...
 private bool TryInline(global::Strict.Bytecode.BinaryExecutable binary, string currentTypeName,
    string currentMethodName,
    Invoke invoke, out List<Instruction> inlinedInstructions)
  {
    inlinedInstructions = [];
    if (!CanInline(binary, currentTypeName, currentMethodName, invoke))
      return false;
    var inlinedExpression = GetInlinedExpression(invoke.Method);
    if (inlinedExpression == null)
      return false;
    var generatedInstructions = global::Strict.Bytecode.BinaryGenerator.GenerateInlineInstructions(
      binary.basePackage,
      inlinedExpression);
  ...
  private static bool CanInline(global::Strict.Bytecode.BinaryExecutable binary,
    string currentTypeName,
  ...
  private static BinaryMethod? FindCompiledMethod(global::Strict.Bytecode.BinaryExecutable binary,
    Method method) =>
  ...
  private static bool IsInlineBlock(IReadOnlyList<Instruction> instructions)
  {
    ...
    for (var index = 0; index < instructions.Count - 1; index++)
      if (instructions[index] is not (LoadVariableToRegister or LoadConstantInstruction or
        BinaryInstruction or ListCallInstruction or Invoke or SetInstruction))
        return false;
    return true;
  }
    ...
    IReadOnlyList<Parameter> parameters, IReadOnlyList<Expression> arguments)
  {
    ...
    return expression switch
    {
      Binary binary => new Binary(SubstituteParameters(binary.Instance!, parameters, arguments),
        binary.Method,
        [SubstituteParameters(binary.Arguments[0], parameters, arguments)]),
      Not not => new Not(not.Method, SubstituteParameters(not.Instance!, parameters, arguments)),
      MethodCall methodCall => new MethodCall(methodCall.Method,
      ...
      ListCall listCall => new ListCall(SubstituteParameters(listCall.List, parameters, arguments),
        SubstituteParameters(listCall.Index, parameters, arguments),
        listCall.SecondIndex != null
          ? SubstituteParameters(listCall.SecondIndex, parameters, arguments)
          : null,
        SubstituteParameters(listCall.OriginalIndex, parameters, arguments)),
      If ifExpression => new If(SubstituteParameters(ifExpression.Condition, parameters, arguments),
  ...
  private static List<Instruction> RemapRegisters(IReadOnlyList<Instruction> instructions,
    global::Strict.Bytecode.Register targetRegister)
  {
    var returnRegister = ((ReturnInstruction)instructions[^1]).Register;
    var map = new Dictionary<global::Strict.Bytecode.Register, global::Strict.Bytecode.Register>
    {
      [returnRegister] = targetRegister
    };
   var nextRegister = ((int)targetRegister + 1) %
      Enum.GetValues<global::Strict.Bytecode.Register>().Length;
    foreach (var register in instructions.SelectMany(GetRegisters))
      if (!map.ContainsKey(register))
      {
       while (map.ContainsValue((global::Strict.Bytecode.Register)nextRegister))
          nextRegister = (nextRegister + 1) %
            Enum.GetValues<global::Strict.Bytecode.Register>().Length;
        map[register] = (global::Strict.Bytecode.Register)nextRegister;
        nextRegister = (nextRegister + 1) %
          Enum.GetValues<global::Strict.Bytecode.Register>().Length;
      }
 ...
 private static IEnumerable<global::Strict.Bytecode.Register> GetRegisters(
  ...
  private static Instruction Clone(Instruction instruction,
   IReadOnlyDictionary<global::Strict.Bytecode.Register,
      global::Strict.Bytecode.Register> registerMap) =>
    instruction switch
    {
